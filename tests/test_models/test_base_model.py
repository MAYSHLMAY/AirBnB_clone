...
class test_for_base_model(unittest.TestCase):
    ...
    def test_attr_none(self):
        """None attribute."""
        obt = BaseModel(None)
        self.assertTrue(hasattr(obt, "id"))
        self.assertTrue(hasattr(obt, "created_at"))
        self.assertTrue(hasattr(obt, "updated_at"))

    def test_kwargs_constructor_2(self):
        """ check id with data """
        dictonary = {'score': 100}

        obt = BaseModel(**dictonary)
        self.assertTrue(hasattr(obt, 'id'))
        self.assertTrue(hasattr(obt, 'created_at'))
        self.assertTrue(hasattr(obt, 'updated_at'))
        self.assertTrue(hasattr(obt, 'score'))
    ...
    def test_str(self):
        ...
        obt = BaseModel(**dictonary)
        out = "[{}] ({}) {}\n".format(type(obt).__name__,
                                      obt.id,
                                      obt.__dict__)

    def test_to_dict(self):
        """ check dict """
        obt = BaseModel(score=300)
        n_dict = obt.to_dict()

        self.assertEqual(n_dict['id'], obt.id)
        self.assertEqual(n_dict['score'], 300)
        self.assertEqual(n_dict['__class__'], 'BaseModel')
        self.assertEqual(n_dict['created_at'], obt.created_at.isoformat())
        self.assertEqual(n_dict['updated_at'], obt.updated_at.isoformat())
